...
func _() {
	...
	// We understand nested loops with various compound statements
	// prior to a go or defer statement.
	for i := range "outer" {
		for j := range "inner" {
			if j < 1 {
				a++
	...
	// We understand various compound statements after a go or defer statement.
	for i := range "outer" {
		for j := range "inner" {
			switch j {
			case 1:
	...
	for i := range "outer" {
		for j := range "inner" {
			switch j {
			case 1: // call foo in a for loop init statement
				go func() {
					print(i)
				}()
				for k := foo(); k < 10; k++ {
			...
			case 2: // call foo in a switch statement tag
				go func() {
					print(i)
				}()
				switch foo() {
			...
			case 3: // call foo in a switch statement case clause expression
				go func() {
					print(i)
				}()
				switch i {
			...
			case 4: // call foo in a type switch init statement
				var a interface{}
				go func() {
					print(i)
				}()
			...
			case 5: // call foo in a type switch case clause body
				var a interface{}
				go func() {
					print(i)
				}()
	...
	// Some additional cases we purposefully disallow.
	...
	// We do not yet understand conversions.
	for i := range "loop" {
		go func() {
			print(i)
		}()
	...
	// We do not yet understand slice literals or map literals.
	for i := range "loop" {
		if i > 1 {
			go func() {
				print(i)
