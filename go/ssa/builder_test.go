...
// TestRangeOverFuncNoPanic ensures that go1.23 range-over-func doesn't
// panic the SSA builder. (Instead, it compiles to a panic instruction.)
// This is a short-term stopgap until Tim's principled fix in CL 555075 lands.
func TestRangeOverFuncNoPanic(t *testing.T) {
	testenv.NeedsGoBuild(t)      // for go/packages
	testenv.NeedsGo1Point(t, 23) // for range-over-func

	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "rangeoverfunc.go"), []byte(`
package main
...
`), 0666); err != nil {
	...
	pkgs, err := loadPackages(dir, "./rangeoverfunc.go")
	if err != nil {
		t.Fatal(err)
	}
	pkg := pkgs[0]

	const mode = ssa.SanityCheckFunctions // add ssa.PrintFunctions to debug
	prog := ssa.NewProgram(pkg.Fset, mode)
	ssapkg := prog.CreatePackage(pkg.Types, pkg.Syntax, pkg.TypesInfo, false)
	ssapkg.Build()

	// Expected body of main:
	//
	// # Name: main.main
	// func main():
	// 0:                                                                entry P:0 S:0
	// 	panic "go1.23 range-over-func (etc)"
	main := ssapkg.Members["main"].(*ssa.Function)
	firstBlock := main.Blocks[0]
	lastInstr := firstBlock.Instrs[len(firstBlock.Instrs)-1]
	if _, ok := lastInstr.(*ssa.Panic); !ok {
		t.Errorf("expected range-over-func to compile to panic, got %T", lastInstr)
	}
}
