...
- When the `semanticTokens` setting is disabled (the default), the server
  no longer advertises the semantic token capability during server initialization.
  This prevents the client from sending unnecessary requests.
