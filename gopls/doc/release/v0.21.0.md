---
title: "Gopls release v0.21.0 (expected Nov 2025)"
---
...
- Also, the experimental `vulncheck` setting now supports the enum value
...
- `minmax` now removes user-defined min/max functions when they are redundant;
- `stringscutprefix` now handles `strings.CutSuffix` too; <!-- #75205 -->
- the `yield` ad `nilness` analyzers handle control-flow booleans more precisely; <!-- #74136, #75924 -->
- `printf` now checks calls to local anonymous printf-wrapper functions too;
...
It also supports the `gopls.vulncheck` tool, allowing agents to scan
packages for security vulnerabilities.
