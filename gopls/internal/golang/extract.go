...
func extractExprs(fset *token.FileSet, start, end token.Pos, src []byte, file *ast.File, info *types.Info, all bool) (*token.FileSet, *analysis.SuggestedFix, error) {
	...
	case *ast.CallExpr:
		...
		if !ok {
			...
			name, _ := generateName(0, "newVar", hasCollision)
			...
			for range tup.Len() {
				...
				var name string
				name, idx = generateName(idx, "newVar", hasCollision)
				lhsNames = append(lhsNames, name)
			}
		}
...
Outer:
	for _, scope := range exprScopes {
		for s := scope; s != info.Scopes[file]; s = s.Parent() {
			counter[s]++
	...
	//   package p
	//   var x = «1 + 2» // error
	insertBeforeStmt, err := stmtToInsertVarBefore(visiblePath, variables)
	if err != nil {
...
func stmtToInsertVarBefore(path []ast.Node, variables []*variable) (ast.Stmt, error) {
	...
	baseIfStmt := func(index int) (ast.Stmt, error) {
		...
		for _, node := range path[index+1:] {
			if ifStmt, ok := node.(*ast.IfStmt); ok && ifStmt.Else == stmt {
	...
	case *ast.CaseClause:
		...
		for _, node := range path[enclosingIndex+1:] {
			if switchStmt, ok := node.(*ast.SwitchStmt); ok {
				...
				if hasFreeVar(typeSwitchStmt.Init) {
...
func canExtractVariable(info *types.Info, file *ast.File, start, end token.Pos, all bool) ([]ast.Expr, error) {
	...
	// Disallow any expr that sits in lhs of an AssignStmt or ValueSpec.
	// (both := and =, change lhs of struct field assign may change semantics).
	for _, e := range exprs {
...
func collectFreeVars(info *types.Info, file *ast.File, start, end token.Pos, node ast.Node) ([]*variable, error) {
	...
	if fileScope == nil {
		return nil, fmt.Errorf("file scope is empty")
	}
	pkgScope := fileScope.Parent()
	if pkgScope == nil {
		return nil, fmt.Errorf("package scope is empty")
	}
