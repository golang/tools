...
func (s *Snapshot) References(ctx context.Context, ids ...PackageID) ([]xrefIndex, error) {
	...
	// Share one objectpath.Encoder across the packages that must be (re)indexed,
	// guarded by a mutex since post may run concurrently. This amortizes
	// object-path encoding so a heavily-referenced package's index is built once.
	var (
