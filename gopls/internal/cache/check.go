...
type typeCheckBatch struct {
	...
	// _syntaxIDs is the set of package IDs that are being fully type-checked (as
	// syntax packages) in this batch. It lets getImportPackage reuse the full
	// type-checked result for an in-batch dependency rather than redundantly
	// type-checking it a second time for import. See getImportPackage.
...
// addSyntaxIDs records the given ids as syntax packages being type-checked in
// this batch (see [typeCheckBatch._syntaxIDs]).
...
// isSyntaxID reports whether id is a syntax package being type-checked in this
// batch.
...
func (b *typeCheckBatch) getImportPackage(ctx context.Context, id PackageID) (pkg *types.Package, err error) {
	return b.importPackages.get(ctx, id, func(ctx context.Context) (*types.Package, error) {
		...
		if err == filecache.ErrNotFound {
			// No cached export data.
			//
			// If this package is also being fully type-checked as a syntax
			// package in this batch, reuse that result rather than redundantly
			// type-checking it a second time for import. This is a significant
			// memory and CPU saving when a change invalidates a large reverse
			// dependency closure (e.g. a syntax error in a core type package):
			// without it, every closure package that is imported by another
			// closure package is parsed and type-checked twice.
			//
			// We only reuse when the package will be checked into the batch's
			// shared FileSet (ph.state < validImports). When ph.state >=
			// validImports, getPackage type-checks into a cloned FileSet, whose
			// positions would not be commensurable with importers using b.fset;
			// in that case fall back to checkPackageForImport, which uses b.fset.
